...
from vgenie.dataset import How2qaDataset, VideoinstructDataset, KineticsDataset, Hmdb51Dataset, MsrvttDataset
...
def build_dataset(is_train, args):
    ...
    if args.data_set == 'CIFAR':
        ...
        dataset = KineticsDataset(
            split='test' if is_train else 'val', # because EVL uses val for evaluation
            base_model_name = base_model_name,
        )
    ...
    elif args.data_set == 'VIDEOINSTRUCT':
        if args.model == 'vit_base_patch16_clip_224.openai':
            base_model_name = 'openai/clip-vit-base-patch16'
            nb_classes = 512
        elif args.model == 'vit_large_patch14_clip_224.openai':
            base_model_name = 'openai/clip-vit-large-patch14'
            nb_classes = 768
        else:
            raise NotImplementedError(f"Unknown model name {args.model}")
        ...
        if is_train:
            ...
            if base_model_name == 'openai/clip-vit-base-patch16':
                dataset = Hmdb51Dataset(
                    split='test',
                    base_model_name=base_model_name,
                    fps=2,
                )
            elif base_model_name == 'openai/clip-vit-large-patch14':
                dataset = How2qaDataset(
                    split='frozenbilm',
                    base_model_name=base_model_name,
                    fps=2,
